...
void _starpu_driver_start_job(struct _starpu_worker *worker, struct _starpu_job *j, struct starpu_perfmodel_arch* perf_arch STARPU_ATTRIBUTE_UNUSED, struct timespec *codelet_start, int rank, int profiling)
{
	...
	unsigned calibrate_power_model = 0;

	if (cl->model && cl->model->benchmarking)
		calibrate_model = 1;
	if (cl->power_model && cl->power_model->benchmarking)
		calibrate_power_model = 1;
	...
	if (rank == 0)
	{
		...
		if ((profiling && profiling_info) || calibrate_model || starpu_top)
		{
			...
			if(mf_is_avail && calibrate_power_model) {
				j->ex_start_ts = ex_starpu_time();
				printf("\n[DEBUG] job starting time is :%Lf\n", j->ex_start_ts);
			}
...
void _starpu_driver_end_job(struct _starpu_worker *worker, struct _starpu_job *j, struct starpu_perfmodel_arch* perf_arch STARPU_ATTRIBUTE_UNUSED, struct timespec *codelet_end, int rank, int profiling)
{
	...
	if (rank == 0)
	{
		if ((profiling && profiling_info) || calibrate_model || starpu_top) {
			...
			if(mf_is_avail && calibrate_power_model) {
				j->ex_end_ts = ex_starpu_time();
				printf("\n[DEBUG] job ending time is :%Lf\n", j->ex_end_ts);
			}
		}
#ifdef HAVE_AYUDAME_H
					...
					struct timespec *codelet_start, struct timespec *codelet_end, int profiling)
{
	...
	if (cl->model && cl->model->benchmarking)
		calibrate_model = 1;
#endif
	/*feed energy from mf to profiling_info->power_consumed*/
	if(mf_is_avail && profiling && profiling_info && cl->power_model && cl->power_model->benchmarking) {
		double energy = (ex_starpu_get_power(j->ex_start_ts, j->ex_end_ts))* (j->ex_end_ts - j->ex_start_ts);
		printf("\n[DEBUG] energy is %f\n", energy);
		if(energy > 0){
			profiling_info->power_consumed = energy;
		}	
	}
